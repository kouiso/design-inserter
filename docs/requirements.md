...
Design Inserter は、[外部サイト](https://pote-chil.com/css-stock/ja) が提供する 222 個の CSS デザインパーツを WordPress の投稿・固定ページにワンクリックで挿入するプラグインである。

外部サイトは見出し・ボタン・ボックス・ローディングアニメーションなど 28 カテゴリのピュア CSS スニペットを公開しており、本プラグインはそれらを WordPress エディタ内で直接選択・プレビュー・挿入できる体験を提供する。
...
- デザインパーツのソースコードはウェブサイト/ブログでの使用を許可
...
| 用語 | 定義 |
|------|------|
| パーツ (Part) | デザインパーツの 1 スニペット（HTML + CSS のペア） |
| カタログ (Catalog) | 全パーツを格納した JSON ファイル |
| カテゴリ (Category) | デザインパーツのスニペット分類（見出し、ボタン等） |
