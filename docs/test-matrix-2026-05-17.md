...
`docs/testing.md` (実行手順 + smoke gate notes) と `scripts/test.mjs` (467 行) を補完する **機能 × 観点 マトリクス** と、その時点の **動作確認結果ログ**。
...
凡例: ✅ = 自動テストで覆われている / 🟡 = 部分的 / ❌ = 未カバー / n/a = 該当しない

### 1. プラグイン読み込み / 初期化

| 機能 | 正常系 | 異常系 | エッジケース | テスト箇所 |
|---|---|---|---|---|
| メインファイル load | ✅ | ❌ ABSPATH 未定義時 exit | n/a | `render-smoke.php:14` |
...
| JSON 不在時 fallback | ✅ | ✅ 空 array 返却 | ✅ sourceUrl 維持 | `catalog-fallback.php missing` |
| JSON 不正時 fallback | ✅ | ✅ 空 array 返却 | n/a | `catalog-fallback.php invalid` |
...
| `sanitize_key` 経由 id 正規化 | ✅ | n/a | ✅ `Heading-1!!` → `heading-1` | `render-smoke.php:65` |
...
| CSS `<style>` タグ出力 | ✅ | n/a | ✅ SVG-only は style 省略 | `render-smoke.php:56, 94-96` |
| 同 id 連続レンダ時の style 重複防止 | ✅ | n/a | ✅ 2 回目は省略 | `render-smoke.php:60-62` |
...
| REST 経由 preview でも同じ scope ルール | ✅ | n/a | n/a | `render-smoke.php:136-142` |
...
| id="存在しない値" 時のフィードバック | ❌ 未カバー (実装は無音で空文字) | ❌ | ❌ | — (前 audit H-10 参照) |
...
| editor.js の React 挙動 (Picker / LivePreview) | ❌ 未カバー (ブラウザ実行必要) | ❌ | ❌ | — (前 audit C-01〜C-06 関連) |
...
| permission_callback (`edit_posts` allow) | ✅ | n/a | n/a | `render-smoke.php:124-126` |
| permission_callback (`edit_posts` deny) | n/a | ✅ false 返却 | n/a | `render-smoke.php:127-128` |
...
| callback 不在 id 時 `WP_Error not_found` | n/a | ✅ | n/a | `render-smoke.php:143-144` |
...
| `scrapedAt` 表示 | ❌ 未実装 (前 audit H-09 参照) | n/a | n/a | — |
...
| 実 DOM 上での behavior 動作 | ❌ 未カバー (ブラウザ実行必要) | ❌ | ❌ | — |
| Esc キーで modal 閉じる | ❌ 未カバー | ❌ | ❌ | — |
| Arrow キーで tabs 移動 | ❌ 未カバー | ❌ | ❌ | — |
...
| zip 内 plugin 単一 root | ✅ | n/a | n/a | (build-plugin-zip.mjs 内) |
...
### 11. WordPress 実 runtime smoke (`npm run smoke:wp:portable`)

| 機能 | カバレッジ | テスト箇所 |
|---|---|---|
| 実 WP-CLI で plugin activate | ✅ | (assertion in `test.mjs:392`) |
...
ローカル WSL (Linux 6.6.87.2) で本ブランチの worktree から `npm test` 実行。検証ソース: **[ローカル実行 (npm test)]**。
...
| 終了 exit code | **0** |
| 失敗 assertion | **0** |
| `test.mjs` の ok 件数 | **50** (うち 1 件が `tests/render-smoke.php` を spawn する PHP smoke 統合) |
| `render-smoke.php` 内部 assertion | **54** (`test.mjs` 視点では 1 ok line に包含) |
...
| Suite | 性質 | 用途 |
|---|---|---|
| `npm test` (本ターン実行) | stub WordPress + 静的 contract test。Docker 不要、軽量 | 全コミット |
| `npm run smoke:wp:portable` | WP-CLI + 実 WordPress 6.9.4 を `/tmp` に展開して plugin activate + shortcode/REST 実 runtime 検証 | release 前 |
| `npm run smoke:wp:docker` | `docker compose` で WP + MySQL を立てて実 runtime 検証 + admin UI 手動 QA 準備 | release 前 |

商用配布フローでは 3 段階全て PASS が理想。本ドキュメント時点で 1 段目のみ検証、2-3 段目は実行環境準備が必要。
...
| **shortcode 不正 id 時のフィードバック** | 投稿者向けエラー表示 (現実装は無音で空文字) | 前 audit H-10 で指摘、修正 PR 提案中 |
| **`scrapedAt` 表示** | 設定ページでのデータ鮮度表示 | 前 audit H-09 で指摘 |
...
| **テーマカラー連動** | WP admin の 9 色テーマで selected 状態が適切表示されるか | 前 audit M-01 で指摘 |

## §D. 次フェーズ提案

1. **Playwright 導入** (PR 別建て): SP 375 / tablet 768 / desktop 1280 の 3 viewport で Docker compose 起動 → WP install → plugin activate → editor 巡回。Picker C-01〜C-06 シナリオ自動化、`@axe-core/playwright` で a11y 違反機械検出
2. **CI workflow 化** (PR 別建て): `.github/workflows/test.yml` で `npm test` を PR ごと自動実行、`smoke:wp:portable` は heavy だが nightly cron 推奨
3. **`docs/test-strategy.md` 体系化** (本ドキュメントを発展): test.mjs の各 testFn と本 doc の §A 行を双方向リンク

---

## メモ

- 本ドキュメントは **test infrastructure (`scripts/test.mjs` / `tests/` / `docs/testing.md` 等) を main にコミットする本 PR と同梱** で初出。前 close した PR #5 は branch context での検証不能を理由に取り下げ済み
- Cross-reference: 前日マージ済 PR #4 `doc/ux-audit-2026-05-16.md` (UX audit) と本ドキュメントは姉妹関係。UX audit は「品質ギャップ発見」、本ドキュメントは「品質測定の網羅性確認」
- ux-audit doc は `doc/` 配下、本 doc は `docs/` 配下に配置。将来 ux-audit doc も `docs/` に移す PR を別途立てる予定 (Gemini PR #5 指摘 #1 への対応)
