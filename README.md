...
- `pnpm run smoke:wp:docker`: Docker コンテナ上で plugin activate / shortcode / dynamic block render を確認
- `pnpm run smoke:wp:portable`: 一時ディレクトリに WP-CLI + WordPress + SQLite drop-in を作成し、plugin activate / shortcode / dynamic block / REST を実 WordPress で確認
- `pnpm run smoke:wp:stubs`: Docker なしで plugin load / hooks / shortcode / dynamic block / REST route を PHP stubs で確認
