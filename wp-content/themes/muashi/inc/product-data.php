 ...
 *
 * Handles data retrieval and formatting for product download pages.
 */

if ( ! function_exists( 'muashi_get_product_download_data' ) ) {
    /**
     * Retrieves and formats product data for the download page application.
     *
     * @return array The structured data for the download page frontend.
     */
    function muashi_get_product_download_data() {
        $max_selectable = 5;
        ...
        foreach ( $taxonomy_config as $taxonomy => $settings ) {
            // function_existsチェックを入れるのが安全だが、functions.phpにあるはず
            $terms = function_exists('muashi_get_sorted_product_terms_flat') 
                ? muashi_get_sorted_product_terms_flat( $taxonomy ) 
                : array();
